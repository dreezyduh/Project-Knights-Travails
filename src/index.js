    ...
    for (let move of posMoves) {
      if (
        ...
        if (newBoard.board[indexToVertex(newPos)].distance === null) {
          newBoard.board[indexToVertex(newPos)].predecessor =
            newBoard.board[indexToVertex(u)];
          newBoard.board[indexToVertex(newPos)].distance =
            newBoard.board[indexToVertex(u)].distance + 1;
          queue.push(newPos);
          if (
            Array.isArray(
              newBoard.adjList[newBoard.board[indexToVertex(u)].value]
            )
          ) {
            newBoard.adjList[newBoard.board[indexToVertex(u)].value].push(
              newBoard.board[indexToVertex(newPos)].value
            );
          } else {
            newBoard.adjList[newBoard.board[indexToVertex(u)].value] = [
              newBoard.board[indexToVertex(newPos)].value,
